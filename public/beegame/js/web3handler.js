...
class Web3Handler {
    ...
    initialize() {
        ...
        if (window.ethereum) {
            ...
            this.tokenContractEthers = new window.ethers.Contract(this.tokenAddress, COFFY_ABI, this.ethersSigner);
    ...
    async startGameOnContract() {
        try {
            ...
            if (!window.ethereum || !this.tokenContractEthers) {
                console.log("Ethers.js contract yok, startGameSession kontrata gönderilmeyecek");
                return false;
            }
            // Cüzdan bağlı mı kontrolü
            const accounts = await window.ethereum.request({ method: 'eth_accounts' });
            if (!accounts || accounts.length === 0) {
                console.log("Cüzdan bağlı değil, startGameSession gönderilmeyecek");
                return false;
            }
            // Ethers.js ile zincire gönder
            console.log("Kontrat üzerinde startGameSession çağrılıyor (ethers.js)...");
            const tx = await this.tokenContractEthers.startGameSession();
            await tx.wait();
            console.log("✅ Kontrat startGameSession başarıyla çağrıldı:", tx.hash);
            return true;
        } catch (error) {
            console.error("Kontrat startGameSession hatası (ethers.js):", error);
            return false;
        }
    }
